    ...
    def __init__(self, setup):
        ...
        match setup.params.get("analytical_solution"):
            ...
            case "cosserat":
                ...
                #    (1, ((x > 2 / nom) | (y > 2 / nom))),
                #    (0.1, ((x < 1 / nom) & (y < 1 / nom))),
                #    (0.1 + 0.9 * sym.Max(nom * x - 1, nom * y - 1), True),
                # )
                cosserat_parameter = cosserat_parameter_base
                # rot = [x * (1 - x) * y * (1 - y), x * (1 - x) * y * (1 - y), x * (1 - x) * y * (1 - y)]
        ...
        source_rot = [
            div_couple_stress[i] - stress_asymmetry[i] for i in range(self.nd)
        ]
        ...
        self.rot = [rot[i] * lame_mu for i in range(self.nd)]
        self.solid_p = (
        ...
        total_rotation = [
            [
                couple_stress[0][0] / 2,
                couple_stress[0][1] / 2 - u[2],
                couple_stress[0][2] / 2 + u[1],
            ],
            [
                couple_stress[1][0] / 2 + u[2],
                couple_stress[1][1] / 2,
                couple_stress[1][2] / 2 - u[0],
            ],
            [
                couple_stress[0][0] / 2 - u[1],
...
class BoundaryConditions(pp.momentum_balance.BoundaryConditionsMomentumBalance):
    ...
    def bc_values_rotation(self, boundary_grid: pp.Grid) -> np.ndarray:
        ...
        if isinstance(boundary_grid, pp.BoundaryGrid):
            if hasattr(self, "exact_sol"):
                return self.exact_sol.rotation(boundary_grid, 0)
            else:
                return np.zeros(boundary_grid.num_cells * self.nd)
